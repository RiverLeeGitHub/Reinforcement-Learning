...
//	#pragma omp parallel for						// Ignore this line. It is the magic that makes the following for-loop happen in parallel.
	for (int trial = 0; trial < numTrials; trial++) {	// Loop over trials
		...
		for (int episode = 0; episode < numEpisodes; episode++) {	// Loop over episodes
			...
			state = environments[trial].getState(generators[trial]);	// Get the initial state.
...
void runMountainCar() {
	...
	int numTrials = 100, numEpisodes = 40, maxEpisodeLength = 20000;	// numTrials = number of agent lifetimes, numEpisodes is the number of episodes in an agent lifetime, and episodes are always terminated if they reach time step maxEpisodeLength. # 100， 40， 20000
	...
	QLearning a1(e.getStateDim(), e.getNumActions(),	0.01,1,0,1,1);// Best hyperparameters for Q-Learning
	Sarsa a2(e.getStateDim(), e.getNumActions(),	0.01,1,0,1,1);// Best hyperparameters for Sarsa
...
void runCartPole() {
	...
	double gamma = 1.0;
	//													alpha		gamma	epsilon	iOrder	dOrder
	QLearning a1(e.getStateDim(), e.getNumActions(),	0.001,0.8,0.2,2,3);// Best hyperparameters for Q-Learning
	Sarsa a2(e.getStateDim(), e.getNumActions(),	1e-05,0.2,0.2,1,5);//0.001,1,0.2,3,3);// Best hyperparameters for Sarsa
	vector<double> means1, vars1, means2, vars2;
	runExperiment(a1, e, numTrials, numEpisodes, maxEpisodeLength, gamma, generator, means1, vars1);
	runExperiment(a2, e, numTrials, numEpisodes, maxEpisodeLength, gamma, generator, means2, vars2);
...
void runAcrobot() {
	...
	//													alpha		gamma	epsilon	iOrder	dOrder
	QLearning a1(e.getStateDim(), e.getNumActions(),	0.01,1,0,1,1);// Best hyperparameters for Q-Learning
	Sarsa a2(e.getStateDim(), e.getNumActions(),		0.0001,1,0,1,5);// Best hyperparameters for Sarsa
	vector<double> means1, vars1, means2, vars2;
	runExperiment(a1, e, numTrials, numEpisodes, maxEpisodeLength, gamma, generator, means1, vars1);
	runExperiment(a2, e, numTrials, numEpisodes, maxEpisodeLength, gamma, generator, means2, vars2);
...
void runGridworld() {
	mt19937_64 generator(0);
	int numTrials = 1000, numEpisodes = 20, maxEpisodeLength = 1000;
	double gamma = 1.0;
	Gridworld e;
	//													alpha		gamma	epsilon	iOrder	dOrder
	QLearning a1(e.getStateDim(), e.getNumActions(),	0.01,0.8,0.2,1,0);// Best hyperparameters for Q-Learning
	Sarsa a2(e.getStateDim(), e.getNumActions(),	0.01,1,0.1,1,0);// Best hyperparameters for Sarsa
