...
vector<vector<double>> multiply2(double a, vector<vector<double>> b){
    for(int i=0; i<b.size(); i++){
...
vector<double> add2(vector<double> a, float b){
    for(int i=0; i<a.size(); i++){
        a[i]+=b;
    }
    return a;
}
vector<vector<double>> add3(vector<vector<double>> a, float b){
    for(int i=0; i<a.size(); i++){
...
vector<vector<double>> add4(vector<vector<double>> a, vector<vector<double>> b){
    for(int i=0; i<a.size(); i++){
        for(int j=0; j<a[0].size(); j++){
            a[i][j]+=b[i][j];
        }
...
// Function for showing matrix when debugging.
...
void QLearning::train(std::mt19937_64 & generator, const std::vector<double> & s, const int & a, double & r, const std::vector<double> & sPrime, const bool & sPrimeTerminal) {
	...
	if (!sPrimeTerminal) {
        ...
        double q = dot(w[a], phi);// Compute q of this step
        double qPrime = maxQ(phiPrime);// Select the maximum value of q
        double TDError = r + gamma * qPrime - q; // Compute TD Error

        for (int i=0; i<w[a].size(); i++){
            w[a][i] = w[a][i] + alpha * TDError * phi[i];// Refresh weights
        }
	}
	else{
	    double q = dot(w[a], phi);
        double qPrime = 0;// Set the qPrime to zero because it is the terminal state.
        double TDError = r + gamma * qPrime - q;

        for (int i=0; i<w[a].size(); i++)
            w[a][i] = w[a][i] + alpha * TDError * phi[i];
	}
...
//  return (uniform_int_distribution<int>(0, (int)bestActions.size() - 1))(generator);	// There are many best actions. Select one uniformly randomly from bestActions.
    return bestActions[(uniform_int_distribution<int>(0, (int)bestActions.size() - 1))(generator)];
}
